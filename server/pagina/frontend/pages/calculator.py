...
try:
    property_price = st.slider("Precio de compra", 0.01, 500000.0, 65000.0, 100.0)
    column1, column2= st.columns(2)
    with column1:
        field1, field2 = st.columns(2)
        salary = field1.number_input("Salario", 0.01, 100000.0, float(st.session_state["salary"]), 100.0)
        savings = field2.number_input("Ahorros", 0.01, 1000000.0, float(st.session_state["savings"]), 100.0)
    with column2:
        field1, field2 = st.columns(2)
    ...
    building_cost = field1.slider("Gasto de obra", 0.01, 100000.0, 0.01, 100.0)
    furnishing_cost =  field2.slider("Gasto de puesta a punto", 0.01, 30000.0, 0.01, 20.0)
    rent = field3.slider("Alquiler esperado", 0.01, 3000.0, 400.0, 100.0)
        
    field1, field2, field3 = st.columns(3)
    expenses = field1.slider("Gastos anuales previstos", 0.01, 3000.0, 350.0, 100.0)
