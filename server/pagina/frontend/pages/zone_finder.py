...
try:
    ...
    with content:
        ...
        if display_option != 'General':
            df = df.reset_index()
            df2 = df[display_option].astype(str).apply(lambda x: eval(x))
            df2 = pd.json_normalize(df2)
            df2 = df2.rename(columns=utils.sell_rent_translation)
            df = pd.concat([df[['Zona', 'Zona Principal', 'Nombre Completo', 'ROI', 'Identificador']], df2], axis=1)
        else:
            df = df.drop(columns=['Alquiler', 'Venta', 'Subzonas', 'Propiedades', '_id'])
