...
try:
    ...
    parent_properties = properties.loc[properties["DB ID"].isin(parent_zone["Propiedades"].iloc[0])]
    properties = properties.loc[properties["ID zona"] == zone["Identificador"]]
    ...
    with col2:
        st.metric(label="ROI", value="{:,.0f} %".format(zone["ROI"]), delta="{:,.0f} %".format(round(zone["ROI"] - parent_zone["ROI"].iloc[0], 2)))
    with col3:
    ...
    with st.expander("Más detalles"):
        ...
        with col1:
            ...
            st.metric(label="Número de propiedades", value="{:,.0f}".format(len(zone["Propiedades"])), delta="{:,.0f}".format(len(zone["Propiedades"]) - len(parent_zone["Propiedades"].iloc[0])))
            st.metric(label="Número de habitaciones medio", value="{:,.0f}".format(zone["Habitaciones"]), delta="{:,.0f}".format(zone["Habitaciones"] - parent_zone["Habitaciones"].iloc[0]))
            st.metric(label="Número de baños medio", value="{:,.0f}".format(zone["Baños"]), delta="{:,.0f}".format(zone["Baños"] - parent_zone["Baños"].iloc[0]))
            st.metric(label="Antigüedad media", value="{:,.0f} días".format(zone["Antigüedad anuncios"], delta="{:,.0f} días".format(zone["Antigüedad anuncios"] - parent_zone["Antigüedad anuncios"].iloc[0])), delta_color="inverse")
            st.metric(label="Número de plantas medio", value="{:,.0f}".format(zone["Planta media"]), delta="{:,.0f}".format(zone["Planta media"] - parent_zone["Planta media"].iloc[0]))
        ...
        with col4:
            ...
            st.metric(label="ITP", value="{:,.0f} €".format(float(zone["ITP"])), delta="{:,.0f} €".format(float(zone["ITP"]) - float(parent_zone["ITP"].iloc[0])), help="Impuesto de transmisiones patrimoniales, varía según la comunidad autónoma entre un 4% y un 10%")
            st.metric(label="Seguro", value="{:,.0f} €".format(float(zone["Seguro"])), delta="{:,.0f} €".format(float(zone["Seguro"]) - float(parent_zone["Seguro"].iloc[0])), help="Seguro de hogar, varía según la compañía y la vivienda")
            st.metric(label="IBI", value="{:,.0f} €".format(float(zone["IBI"])), delta="{:,.0f} €".format(float(zone["IBI"]) - float(parent_zone["IBI"].iloc[0])), help="Impuesto de bienes inmuebles, se situa entre un 0.4% y un 1.3% según la vivienda")
        with col5:
            st.markdown("##### Media gastos anuales")
            st.metric(label="Comunidad", value="{:,.0f} €".format(float(zone["Comunidad"])), delta="{:,.0f} €".format(float(zone["Comunidad"]) - float(parent_zone["Comunidad"].iloc[0])), help="Gastos de comunidad de vecinos")
            st.metric(label="Mantenimiento", value="{:,.0f} €".format(float(zone["Mantenimiento"])), delta="{:,.0f} €".format(float(zone["Mantenimiento"]) - float(parent_zone["Mantenimiento"].iloc[0])), help="Gastos de mantenimiento de la vivienda")
            st.metric(label="Total con impuestos", value="{:,.0f} €".format(float(zone["Gastos mensuales"])*12), delta="{:,.0f} €".format(float(zone["Gastos mensuales"])*12 - float(parent_zone["Gastos mensuales"].iloc[0])*12))
        with col6:
            st.markdown("##### Alquiler medio")
            st.metric(label="Ingresos", value="{:,.0f} €".format(float(zone["Ingresos esperados"])), delta="{:,.0f} €".format(float(zone["Ingresos esperados"]) - float(parent_zone["Ingresos esperados"].iloc[0])))
